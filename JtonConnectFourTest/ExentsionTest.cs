...
namespace ExentsionTest
{
    public class UnitTest
    {
        ...
        public void Setup()
        {
            ...
            _substrateClient.RegisterTypeConverter(new GenericTypeConverter<PlayerStruct>());
        }
        ...
        public async Task NewGameTestAsync()
        {
            ...
            // 
            _ = await _substrateClient.Author.SubmitAndWatchExtrinsicAsync(actionExtrinsicUpdate, JtonConnectFourCall.NewGame(Bob.Value), Alice, 0, 64, cts.Token);
            Thread.Sleep(extrinsic_wait);
            ...
            //  ]
            //},
            var board_id_a = await _substrateClient.GetStorageAsync("ConnectFour", "PlayerBoard", new[] { Utils.Bytes2HexString(Alice.Bytes) }, cts.Token);
            Assert.AreEqual("Hash", board_id_a.GetType().Name);

            var board_id_b = await _substrateClient.GetStorageAsync("ConnectFour", "PlayerBoard", new[] { Utils.Bytes2HexString(Bob.Bytes) }, cts.Token);
            ...
            Assert.AreEqual(Alice.Value, boardStruct1.Red.Value);
            Assert.AreEqual(Bob.Value, boardStruct1.Blue.Value);
            ...
            {
                _ = await _substrateClient.Author.SubmitAndWatchExtrinsicAsync(actionExtrinsicUpdate, JtonConnectFourCall.PlayTurn(0), Alice, 0, 64, cts.Token);
                Thread.Sleep(extrinsic_wait);
                player1Start = true;
            }

            _ = await _substrateClient.Author.SubmitAndWatchExtrinsicAsync(actionExtrinsicUpdate, JtonConnectFourCall.PlayTurn(1), Bob, 0, 64, cts.Token);
            Thread.Sleep(extrinsic_wait);
